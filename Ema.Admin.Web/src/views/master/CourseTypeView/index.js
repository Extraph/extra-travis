...
const CourseTypeView = () => {
  ...
  const handleRowUpdate = async (newData, oldData, resolve) => {
    ...
    //validation
    let errorList = [];
    if (newData.courseType === undefined) {
      errorList.push('Please enter course type.');
    }

    if (errorList.length < 1) {
      await adminIJoin
        .put(`/CourseType/${newData.id}`, newData)
        .then((res) => {
        ...
        .catch((error) => {
          if (error.response.status === 409) {
            setErrorMessages([
              `${newData.courseType} course type already exists.`
            ]);
          } else {
  ...
  return (
    <React.Fragment>
      <Page className={classes.root} title="Course Type Management">
